...
class _WatchlistScreenState extends State<WatchlistScreen> {
  ...
  Future<void> _fetchWatchlistMovies(String accountId, String sessionId) async {
    ...
    final moviesResponse = await http.get(
      Uri.parse(
          'https://api.themoviedb.org/3/account/$accountId/watchlist/movies?api_key=$apiKey&session_id=$sessionId&sort_by=created_at.desc&language=$languageCode'),
    );

    if (moviesResponse.statusCode == 200) {
      final moviesData = jsonDecode(moviesResponse.body);
      setState(() {
        _watchlistMovies = moviesData['results'];
      });
    } else {
      _showErrorDialog();
    }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
              'Không thể lấy danh sách phim trong watchlist hoặc thông tin người dùng.'),
          actions: [
            TextButton(
              onPressed: () {
  ...
  void initState() {
    ...
    if (sessionId.isNotEmpty && accountId.isNotEmpty) {
      _fetchWatchlistMovies(accountId, sessionId);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // UserHeader(userName: _userName), // Sử dụng UserHeader trong phần thân
          Text(
          ...
          Expanded(
            child: _watchlistMovies.isNotEmpty
                ? ListView.builder(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  CustomIconButton(
                    icon: Icons.favorite,
                    text: isVietnameseMode ? "Yêu thích" : 'Favorite',
                    iconBackgroundColor: Colors.white,
                    iconColor: Colors.red,
                    textColor: isDarkMode ? Colors.white : Colors.black,
                  ),
